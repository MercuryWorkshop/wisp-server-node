                    ...
                    for (const [_, stream] of connections.entries()) {
                        ...
                        const interval =
                            options.throttleInterval !== undefined
                                ...
                                : defaultOptions.throttleInterval;

                        // @ts-ignore - We are explicitly checking if throttleLimit is undefined
                        const limit =
                            options.throttleLimit !== undefined ? options.throttleLimit : defaultOptions.throttleLimit;
                            ...
                            if (stream.dataTransferred >= limit!) {
                                if (options.logging) {
                                    if (stream.client instanceof net.Socket) {
                                        console.log(
                                            `Throttling stream ID ${wispFrame.streamID} from ${stream.client.remoteAddress}:${stream.client.remotePort} due to excessive data transfer (elapsed time: ${now - stream.lastThrottleCheck}ms, data transferred: ${stream.dataTransferred} bytes)`,
                                        );
                                    } else if (stream.client instanceof dgram.Socket) {
                                        console.log(
                                            `Throttling stream ID ${wispFrame.streamID} from ${stream.client.address}:${stream.client.port} due to excessive data transfer (elapsed time: ${now - stream.lastThrottleCheck}ms, data transferred: ${stream.dataTransferred} bytes)`,
                                        );
                                    }
                                }
